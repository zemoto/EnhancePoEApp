...
namespace EnhancePoE.Model.Storage
{
   public class OnlineFilterStorage : IFilterStorage
   {
      ...
      private const string FilterUrlXPathTemplate = "//div[@class='item-filter' and h3/a/text()='{0}']/div[@class='item-filter-buttons']/a[@class='button-text' and text()='Edit']";
      ...
      public async Task<string> ReadLootFilterAsync()
      {
         ...
         {
            _ = System.Windows.MessageBox.Show( "No lootfilter found! Please check that you set the correct name and that you use your own filter. You cannot use followed filters.", "Could not read Lootfilter", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error );
            return null;
         }
      ...
      public async Task WriteLootFilterAsync( string filter )
      {
         ...
         using ( var client = new HttpClient( handler ) )
         {
            ...
            _ = await client.PostAsync( filterUrl, content );
